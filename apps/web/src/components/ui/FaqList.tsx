  ...
  return (
    <>
      <div className={cn('flex flex-col gap-4', className)}>
        {items.map((item) => (
          <details
            className="group rounded-card border border-line bg-white p-6 open:border-jade-200"
            key={item.key}
          >
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-lg font-bold text-ink [&::-webkit-details-marker]:hidden">
              ...
              <span
                aria-hidden
                className="inline-block shrink-0 text-2xl font-black text-jade-500 transition-transform group-open:rotate-45"
              >
                +
              </span>
            </summary>
