/**
 * A session-scoped identifier, not a persistent account. Nothing in this app
 * persists across a relaunch yet — `AppStateProvider`'s own `facts` state is
 * plain in-memory `useState`, the same as this — and there is still no
