...
export default function TwinScreen() {
  ...
  return <Screen>
    <SectionTitle eyebrow="PATIENT-CONTROLLED" title={language === 'en' ? 'My health picture' : 'मेरो स्वास्थ्य चित्र'} body={language === 'en' ? 'A transparent picture of facts — not a diagnosis, a prediction, or a claim of completeness.' : 'तथ्यहरूको पारदर्शी चित्र—निदान, भविष्यवाणी वा पूर्णताको दाबी होइन।'} />
    <View style={styles.progress}><View style={styles.progressHead}><Text style={styles.progressTitle}>{language === 'en' ? "Today's useful foundations" : 'आजका उपयोगी आधारहरू'}</Text><Text style={styles.progressValue}>{progress}%</Text></View><View style={styles.track}><View style={[styles.fill, { width: (progress + '%') as DimensionValue }]} /></View><Text style={styles.hint}>{language === 'en' ? 'This is not a treatment score. You may skip any question.' : 'यो उपचारको स्कोर होइन। कुनै प्रश्न छोड्न सक्नुहुन्छ।'}</Text></View>
    {prompt ? <View style={styles.card}><View style={styles.icon}><Sparkles color={colors.primary} /></View><Text style={styles.question}>{language === 'en' ? prompt.questionEn : prompt.questionNe}</Text><View style={styles.why}><Text style={styles.whyLabel}>{language === 'en' ? 'Why is this asked?' : 'किन सोधिएको?'}</Text><Text style={styles.whyText}>{language === 'en' ? prompt.whyEn : prompt.whyNe}</Text></View><TextInput value={answer} onChangeText={setAnswer} placeholder={language === 'en' ? 'Write here…' : 'यहाँ लेख्नुहोस्…'} placeholderTextColor="#82918E" style={styles.input} /><Pressable disabled={!answer.trim()} onPress={save} style={[uiStyles.primaryButton, !answer.trim() && { opacity: 0.38 }]}><Text style={uiStyles.primaryButtonText}>{language === 'en' ? 'Add to my picture' : 'मेरो चित्रमा राख्नुहोस्'}</Text></Pressable><Pressable onPress={() => skipPrompt(prompt.id)} style={styles.skip}><Text style={styles.skipText}>{language === 'en' ? 'Skip for now' : 'अहिले छोड्नुहोस्'}</Text></Pressable><View style={styles.consent}><LockKeyhole color={colors.muted} size={16} /><Text style={styles.hint}>{language === 'en' ? 'This answer is not shared with anyone right now.' : 'यो उत्तर अहिले कसैसँग साझा हुँदैन।'}</Text></View></View> : null}
    <Text style={styles.factHeading}>{language === 'en' ? "Facts you've confirmed" : 'तपाईंले पुष्टि गरेका तथ्य'}</Text>
    {facts.length === 0 ? <Text style={styles.empty}>{language === 'en' ? 'No facts added yet. Start with one easy step.' : 'अहिलेसम्म कुनै तथ्य राखिएको छैन। एउटा सजिलो कदमबाट सुरु गर्नुहोस्।'}</Text> : facts.map((fact) => <View key={fact.id} style={styles.fact}><View style={styles.check}><Check color="white" size={14} /></View><View style={{ flex: 1 }}><Text style={styles.factLabel}>{fact.label}</Text><Text style={styles.factValue}>{fact.value}</Text><Text style={styles.factMeta}>{language === 'en' ? 'You reported · Confirmed' : 'तपाईंले बताउनुभएको · पुष्टि गरिएको'}</Text></View></View>)}
  </Screen>;
}
