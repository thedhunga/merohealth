...
| ~~`/app` 404~~ | Stale as of 2026-08-22 — re-verified by running the exact production build command (`scripts/vercel-build.sh`, what `apps/web/vercel.json`'s `buildCommand` calls) from a clean tree, then serving the built `@swasthya/web` output locally: `GET /app` returns `200` with the real 87 KB Expo `index.html`, not a 404. The Round three B3 fix works. Left the row rather than deleting it — this confirms the local build pipeline, not the live Vercel deployment, which this environment has no network access to check directly. | If `/app` still 404s on the live site despite this, the gap is Vercel project config (Root Directory, a stale deploy) — not the build script. |
